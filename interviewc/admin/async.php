...
if(isset($params['method']) && $params['method'] == 'anc'){
  ...
  $conditions = "WHERE status IN ('1') AND mstatus IN ('0','1','3') ".$search.$position_filter." ORDER BY ".$columnsMap[$order['column']]." ".$order['dir'];
  ...
  while ($contact = mysql_fetch_array($pack)){
    ...
    $result[] = array(
      ...
      $contact['status'], $contact['mstatus'], $fetch['mstatus'], intval($numrows), $contact['id'], ord($contact['nstatus'])
    );
  }
